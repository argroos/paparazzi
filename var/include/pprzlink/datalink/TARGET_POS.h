 ...
 * @param _course 
 * @param _heading Ground heading
 */
static inline void pprzlink_msg_v2_send_TARGET_POS(struct pprzlink_msg * msg, uint8_t *_ac_id, uint8_t *_target_id, int32_t *_lat, int32_t *_lon, int32_t *_alt, float *_speed, float *_climb, float *_course, float *_heading) {
#if PPRZLINK_ENABLE_FD
  long _FD = 0; /* can be an address, an index, a file descriptor, ... */
#endif
  const uint8_t size = msg->trans->size_of(msg, /* msg header overhead */4+1+1+4+4+4+4+4+4+4);
  if (msg->trans->check_available_space(msg, _FD_ADDR, size)) {
    msg->trans->count_bytes(msg, size);
    msg->trans->start_message(msg, _FD, /* msg header overhead */4+1+1+4+4+4+4+4+4+4);
...
#define DOWNLINK_SEND_TARGET_POS(_trans, _dev, ac_id, target_id, lat, lon, alt, speed, climb, course, heading) pprz_msg_send_TARGET_POS(&((_trans).trans_tx), &((_dev).device), AC_ID, ac_id, target_id, lat, lon, alt, speed, climb, course, heading)
/**
 ...
 * @param _heading Ground heading
 */
static inline void pprz_msg_send_TARGET_POS(struct transport_tx *trans, struct link_device *dev, uint8_t ac_id, uint8_t *_ac_id, uint8_t *_target_id, int32_t *_lat, int32_t *_lon, int32_t *_alt, float *_speed, float *_climb, float *_course, float *_heading) {
    ...
    msg.receiver_id = 0;
    msg.component_id = 0;
    pprzlink_msg_v2_send_TARGET_POS(&msg,_ac_id,_target_id,_lat,_lon,_alt,_speed,_climb,_course,_heading);
}


#else // DOWNLINK

#define DOWNLINK_SEND_TARGET_POS(_trans, _dev, ac_id, target_id, lat, lon, alt, speed, climb, course, heading) {}
static inline void pprz_send_msg_TARGET_POS(struct transport_tx *trans __attribute__((unused)), struct link_device *dev __attribute__((unused)), uint8_t ac_id __attribute__((unused)), uint8_t *_ac_id __attribute__((unused)), uint8_t *_target_id __attribute__((unused)), int32_t *_lat __attribute__((unused)), int32_t *_lon __attribute__((unused)), int32_t *_alt __attribute__((unused)), float *_speed __attribute__((unused)), float *_climb __attribute__((unused)), float *_course __attribute__((unused)), float *_heading __attribute__((unused))) {}
...
/** Getter for field heading in message TARGET_POS
  *
  * @param _payload : a pointer to the TARGET_POS message
  * @return Ground heading
  */
static inline float pprzlink_get_DL_TARGET_POS_heading(uint8_t * _payload __attribute__((unused)))
{
...
#define DL_TARGET_POS_heading(_payload) pprzlink_get_DL_TARGET_POS_heading(_payload)
