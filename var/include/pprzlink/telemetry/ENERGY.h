 ...
 * @param _avg_power Average electrical power
 * @param _charge Accumulated consumed charge
 * @param _energy Accumulated consumed energy
 */
static inline void pprzlink_msg_v2_send_ENERGY(struct pprzlink_msg * msg, uint8_t *_throttle, float *_voltage, float *_current, float *_power, float *_avg_power, float *_charge, float *_energy) {
#if PPRZLINK_ENABLE_FD
  long _FD = 0; /* can be an address, an index, a file descriptor, ... */
#endif
  const uint8_t size = msg->trans->size_of(msg, /* msg header overhead */4+1+4+4+4+4+4+4);
  if (msg->trans->check_available_space(msg, _FD_ADDR, size)) {
    msg->trans->count_bytes(msg, size);
    msg->trans->start_message(msg, _FD, /* msg header overhead */4+1+4+4+4+4+4+4);
    ...
    msg->trans->put_bytes(msg, _FD, DL_TYPE_FLOAT, DL_FORMAT_SCALAR, (void *) _avg_power, 4);
...
#define DOWNLINK_SEND_ENERGY(_trans, _dev, throttle, voltage, current, power, avg_power, charge, energy) pprz_msg_send_ENERGY(&((_trans).trans_tx), &((_dev).device), AC_ID, throttle, voltage, current, power, avg_power, charge, energy)
/**
 ...
 * @param _energy Accumulated consumed energy
 */
static inline void pprz_msg_send_ENERGY(struct transport_tx *trans, struct link_device *dev, uint8_t ac_id, uint8_t *_throttle, float *_voltage, float *_current, float *_power, float *_avg_power, float *_charge, float *_energy) {
    ...
    msg.receiver_id = 0;
    msg.component_id = 0;
    pprzlink_msg_v2_send_ENERGY(&msg,_throttle,_voltage,_current,_power,_avg_power,_charge,_energy);
}


#else // DOWNLINK

#define DOWNLINK_SEND_ENERGY(_trans, _dev, throttle, voltage, current, power, avg_power, charge, energy) {}
static inline void pprz_send_msg_ENERGY(struct transport_tx *trans __attribute__((unused)), struct link_device *dev __attribute__((unused)), uint8_t ac_id __attribute__((unused)), uint8_t *_throttle __attribute__((unused)), float *_voltage __attribute__((unused)), float *_current __attribute__((unused)), float *_power __attribute__((unused)), float *_avg_power __attribute__((unused)), float *_charge __attribute__((unused)), float *_energy __attribute__((unused))) {}
...
/** Getter for field avg_power in message ENERGY
  *
  * @param _payload : a pointer to the ENERGY message
  * @return Average electrical power
  */
static inline float pprzlink_get_DL_ENERGY_avg_power(uint8_t * _payload __attribute__((unused)))
{
...
#define DL_ENERGY_avg_power(_payload) pprzlink_get_DL_ENERGY_avg_power(_payload)
