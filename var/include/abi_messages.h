/* Automatically generated by gen_abi from /home/argroos/paparazzi/conf/abi.xml */
...
#define ABI_IMU_GYRO_ID 4
#define ABI_IMU_ACCEL_ID 5
#define ABI_IMU_MAG_ID 6
...
#define ABI_RADIO_CONTROL_ID 30
#define ABI_IMU_GYRO_RAW_ID 31
#define ABI_IMU_ACCEL_RAW_ID 32
#define ABI_IMU_MAG_RAW_ID 33
#define ABI_IMU_GYRO_INT_ID 34
#define ABI_IMU_ACCEL_INT_ID 35
#define ABI_VEL_SP_ID 36
#define ABI_LIDAR_DATA_ID 37

/* Array and linked list structure */
#define ABI_MESSAGE_NB 38
...
typedef void (*abi_callbackIMU_GYRO)(uint8_t sender_id, uint32_t stamp, struct Int32Rates * gyro);
typedef void (*abi_callbackIMU_ACCEL)(uint8_t sender_id, uint32_t stamp, struct Int32Vect3 * accel);
typedef void (*abi_callbackIMU_MAG)(uint8_t sender_id, uint32_t stamp, struct Int32Vect3 * mag);
...
typedef void (*abi_callbackACT_FEEDBACK)(uint8_t sender_id, struct act_feedback_t * feedback, uint8_t num_act);
typedef void (*abi_callbackTHRUST)(uint8_t sender_id, struct FloatVect3 thrust_increment_vect);
...
typedef void (*abi_callbackRADIO_CONTROL)(uint8_t sender_id, struct RadioControl * rc);
typedef void (*abi_callbackIMU_GYRO_RAW)(uint8_t sender_id, uint32_t stamp, struct Int32Rates * gyro, uint8_t samples, float rate, float temp);
typedef void (*abi_callbackIMU_ACCEL_RAW)(uint8_t sender_id, uint32_t stamp, struct Int32Vect3 * accel, uint8_t samples, float rate, float temp);
typedef void (*abi_callbackIMU_MAG_RAW)(uint8_t sender_id, uint32_t stamp, struct Int32Vect3 * mag);
typedef void (*abi_callbackIMU_GYRO_INT)(uint8_t sender_id, uint32_t stamp, struct FloatRates * delta_gyro, uint16_t dt);
...
static inline void AbiBindMsgIMU_GYRO(uint8_t sender_id, abi_event * ev, abi_callbackIMU_GYRO cb) {
  if (abi_queues[ABI_IMU_GYRO_ID] == ev) return;
  ev->id = sender_id;
  ev->cb = (abi_callback)cb;
  ABI_PREPEND(abi_queues[ABI_IMU_GYRO_ID],ev);
}

static inline void AbiSendMsgIMU_GYRO(uint8_t sender_id, uint32_t stamp, struct Int32Rates * gyro) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_IMU_GYRO_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackIMU_GYRO cb = (abi_callbackIMU_GYRO)(e->cb);
      cb(sender_id, stamp, gyro);
    }
  }
}

static inline void AbiBindMsgIMU_ACCEL(uint8_t sender_id, abi_event * ev, abi_callbackIMU_ACCEL cb) {
  if (abi_queues[ABI_IMU_ACCEL_ID] == ev) return;
  ev->id = sender_id;
  ev->cb = (abi_callback)cb;
  ABI_PREPEND(abi_queues[ABI_IMU_ACCEL_ID],ev);
}

static inline void AbiSendMsgIMU_ACCEL(uint8_t sender_id, uint32_t stamp, struct Int32Vect3 * accel) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_IMU_ACCEL_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackIMU_ACCEL cb = (abi_callbackIMU_ACCEL)(e->cb);
      cb(sender_id, stamp, accel);
    }
  }
}

static inline void AbiBindMsgIMU_MAG(uint8_t sender_id, abi_event * ev, abi_callbackIMU_MAG cb) {
  if (abi_queues[ABI_IMU_MAG_ID] == ev) return;
  ev->id = sender_id;
  ev->cb = (abi_callback)cb;
  ABI_PREPEND(abi_queues[ABI_IMU_MAG_ID],ev);
}

static inline void AbiSendMsgIMU_MAG(uint8_t sender_id, uint32_t stamp, struct Int32Vect3 * mag) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_IMU_MAG_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackIMU_MAG cb = (abi_callbackIMU_MAG)(e->cb);
...
static inline void AbiBindMsgACT_FEEDBACK(uint8_t sender_id, abi_event * ev, abi_callbackACT_FEEDBACK cb) {
  if (abi_queues[ABI_ACT_FEEDBACK_ID] == ev) return;
  ev->id = sender_id;
  ev->cb = (abi_callback)cb;
  ABI_PREPEND(abi_queues[ABI_ACT_FEEDBACK_ID],ev);
}

static inline void AbiSendMsgACT_FEEDBACK(uint8_t sender_id, struct act_feedback_t * feedback, uint8_t num_act) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_ACT_FEEDBACK_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackACT_FEEDBACK cb = (abi_callbackACT_FEEDBACK)(e->cb);
      cb(sender_id, feedback, num_act);
    }
  }
}
...
static inline void AbiSendMsgTHRUST(uint8_t sender_id, struct FloatVect3 thrust_increment_vect) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_THRUST_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackTHRUST cb = (abi_callbackTHRUST)(e->cb);
      cb(sender_id, thrust_increment_vect);
    }
  }
}
...
static inline void AbiBindMsgRADIO_CONTROL(uint8_t sender_id, abi_event * ev, abi_callbackRADIO_CONTROL cb) {
  if (abi_queues[ABI_RADIO_CONTROL_ID] == ev) return;
  ev->id = sender_id;
  ev->cb = (abi_callback)cb;
  ABI_PREPEND(abi_queues[ABI_RADIO_CONTROL_ID],ev);
}

static inline void AbiSendMsgRADIO_CONTROL(uint8_t sender_id, struct RadioControl * rc) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_RADIO_CONTROL_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackRADIO_CONTROL cb = (abi_callbackRADIO_CONTROL)(e->cb);
      cb(sender_id, rc);
    }
  }
}

static inline void AbiBindMsgIMU_GYRO_RAW(uint8_t sender_id, abi_event * ev, abi_callbackIMU_GYRO_RAW cb) {
  if (abi_queues[ABI_IMU_GYRO_RAW_ID] == ev) return;
  ev->id = sender_id;
  ev->cb = (abi_callback)cb;
  ABI_PREPEND(abi_queues[ABI_IMU_GYRO_RAW_ID],ev);
}

static inline void AbiSendMsgIMU_GYRO_RAW(uint8_t sender_id, uint32_t stamp, struct Int32Rates * gyro, uint8_t samples, float rate, float temp) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_IMU_GYRO_RAW_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackIMU_GYRO_RAW cb = (abi_callbackIMU_GYRO_RAW)(e->cb);
      cb(sender_id, stamp, gyro, samples, rate, temp);
    }
  }
}

static inline void AbiBindMsgIMU_ACCEL_RAW(uint8_t sender_id, abi_event * ev, abi_callbackIMU_ACCEL_RAW cb) {
  if (abi_queues[ABI_IMU_ACCEL_RAW_ID] == ev) return;
  ev->id = sender_id;
  ev->cb = (abi_callback)cb;
  ABI_PREPEND(abi_queues[ABI_IMU_ACCEL_RAW_ID],ev);
}

static inline void AbiSendMsgIMU_ACCEL_RAW(uint8_t sender_id, uint32_t stamp, struct Int32Vect3 * accel, uint8_t samples, float rate, float temp) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_IMU_ACCEL_RAW_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackIMU_ACCEL_RAW cb = (abi_callbackIMU_ACCEL_RAW)(e->cb);
      cb(sender_id, stamp, accel, samples, rate, temp);
    }
  }
}

static inline void AbiBindMsgIMU_MAG_RAW(uint8_t sender_id, abi_event * ev, abi_callbackIMU_MAG_RAW cb) {
  if (abi_queues[ABI_IMU_MAG_RAW_ID] == ev) return;
  ev->id = sender_id;
  ev->cb = (abi_callback)cb;
  ABI_PREPEND(abi_queues[ABI_IMU_MAG_RAW_ID],ev);
}

static inline void AbiSendMsgIMU_MAG_RAW(uint8_t sender_id, uint32_t stamp, struct Int32Vect3 * mag) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_IMU_MAG_RAW_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackIMU_MAG_RAW cb = (abi_callbackIMU_MAG_RAW)(e->cb);
      cb(sender_id, stamp, mag);
    }
  }
}

static inline void AbiBindMsgIMU_GYRO_INT(uint8_t sender_id, abi_event * ev, abi_callbackIMU_GYRO_INT cb) {
  if (abi_queues[ABI_IMU_GYRO_INT_ID] == ev) return;
  ev->id = sender_id;
  ev->cb = (abi_callback)cb;
  ABI_PREPEND(abi_queues[ABI_IMU_GYRO_INT_ID],ev);
}

static inline void AbiSendMsgIMU_GYRO_INT(uint8_t sender_id, uint32_t stamp, struct FloatRates * delta_gyro, uint16_t dt) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_IMU_GYRO_INT_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackIMU_GYRO_INT cb = (abi_callbackIMU_GYRO_INT)(e->cb);
      cb(sender_id, stamp, delta_gyro, dt);
    }
  }
}

static inline void AbiBindMsgIMU_ACCEL_INT(uint8_t sender_id, abi_event * ev, abi_callbackIMU_ACCEL_INT cb) {
  if (abi_queues[ABI_IMU_ACCEL_INT_ID] == ev) return;
  ev->id = sender_id;
  ev->cb = (abi_callback)cb;
  ABI_PREPEND(abi_queues[ABI_IMU_ACCEL_INT_ID],ev);
}

static inline void AbiSendMsgIMU_ACCEL_INT(uint8_t sender_id, uint32_t stamp, struct FloatVect3 * delta_accel, uint16_t dt) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_IMU_ACCEL_INT_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackIMU_ACCEL_INT cb = (abi_callbackIMU_ACCEL_INT)(e->cb);
      cb(sender_id, stamp, delta_accel, dt);
    }
  }
}

static inline void AbiBindMsgVEL_SP(uint8_t sender_id, abi_event * ev, abi_callbackVEL_SP cb) {
  if (abi_queues[ABI_VEL_SP_ID] == ev) return;
  ev->id = sender_id;
  ev->cb = (abi_callback)cb;
  ABI_PREPEND(abi_queues[ABI_VEL_SP_ID],ev);
}

static inline void AbiSendMsgVEL_SP(uint8_t sender_id, struct FloatVect3 * vel_sp) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_VEL_SP_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackVEL_SP cb = (abi_callbackVEL_SP)(e->cb);
      cb(sender_id, vel_sp);
    }
  }
}

static inline void AbiBindMsgLIDAR_DATA(uint8_t sender_id, abi_event * ev, abi_callbackLIDAR_DATA cb) {
  if (abi_queues[ABI_LIDAR_DATA_ID] == ev) return;
  ev->id = sender_id;
  ev->cb = (abi_callback)cb;
  ABI_PREPEND(abi_queues[ABI_LIDAR_DATA_ID],ev);
}

static inline void AbiSendMsgLIDAR_DATA(uint8_t sender_id, uint32_t stamp, uint32_t numRows, uint32_t numCols, uint16_t size, uint8_t subtype, uint8_t* data) {
  abi_event* e;
  ABI_FOREACH(abi_queues[ABI_LIDAR_DATA_ID],e) {
    if (e->id == ABI_BROADCAST || e->id == sender_id) {
      abi_callbackLIDAR_DATA cb = (abi_callbackLIDAR_DATA)(e->cb);
      cb(sender_id, stamp, numRows, numCols, size, subtype, data);
    }
  }
}
